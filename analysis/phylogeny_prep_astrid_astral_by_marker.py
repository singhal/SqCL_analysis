...
def get_args():
	...
	parser.add_argument(
        ...
        parser.add_argument(
                ...
                type=str,
                default=None,
                help='AHE | UCE | gene -- capitalization insensitive.'
                )
...
def get_sp_loci(args):
	...
	loci = [x for x in loci 
               if re.search(args.marker.upper(), x.upper())]
...
def create_files(args, dir, loci):
	...
	out = os.path.join(outdir, 'best_trees_miss%s_tol%s_collapse%s_%s.trees' % 
                          (args.miss, args.tol, args.collapse, args.marker))
	bs = os.path.join(outdir, 'bootstrap_files_miss%s_tol%s_collapse%s_%s.txt' % 
                         (args.miss, args.tol, args.collapse, args.marker))
	bs_out = open(bs, 'w')
