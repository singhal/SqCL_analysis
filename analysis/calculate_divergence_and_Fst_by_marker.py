...
def get_divergence(lineage, inds, vcf, outdir):
	...
	for ix, ind1 in enumerate(inds):
		...
		for ind2 in inds[(ix + 1):]:
			div[ind1][ind2] = {}
			for type in types:
				div[ind1][ind2][type] = {'diff': 0, 'denom': 0}
			...
			if len(re.split(',', d[4])) == 1:
				...
				if d[4] in ['A', 'T', 'C', 'G']:
					for ind, geno in zip(inds, genos):
						counts[ind][type].append(count[geno])
				...
				for ind1 in div:
					for ind2 in div[ind1]:
						if genos[ind1] != './.' and genos[ind2] != './.':
							div[ind1][ind2][type]['denom'] += 1
							div[ind1][ind2][type]['diff'] += diff[genos[ind1]][genos[ind2]]
	f.close()


	out = os.path.join(outdir, '%s_divergence.csv' % lineage)
	o = open(out, 'w')
	o.write('lineage,type,ind1,ind2,nuc_dxy,nuc_denom,fst,fst_denom\n')
	for ind1 in div:
		for ind2 in div[ind1]:
			for type in types:
				dxy_denom = div[ind1][ind2][type]['denom']
				if dxy_denom > 0:
					dxy = div[ind1][ind2][type]['diff'] / float(div[ind1][ind2][type]['denom'])
				else:
					dxy = np.nan

				alleles = np.array([counts[ind1][type], counts[ind2][type]])
				...
				o.write('%s,%s,%s,%s,%.6f,%s,%.6f,%s\n' % 
					(lineage, type, ind1, ind2, dxy, dxy_denom, fst, len(alleles[0])))
